...
!0;var c={content:{contentId:b.contentId}};"V"===b.contentType?a.Model.Trail.videos.push(c):a.Model.Trail.resources.push(c)}});d||(a.RootModel.modalStatus="Please first select some items to add!")};a.saveTrail=function(d){c.post("/trail/saveTrail.a",a.Model.Trail).success(function(b){var f="Saved the trail!";"OK"===b.Status?a.Model.Trail=b.Message:f="Failed to save the trail!";d?a.RootModel.modalStatus=f:a.setStatus(f)})};a.createAssessFromQ=function(){var c=[];angular.forEach(a.Model.PF.questionBank,
function(a){qq={questionId:a.questionId,title:a.title,description:a.description};a.selected&&c.push(qq)});g.aqList=c;e.path("/Assessment")}}]);
srsApp.controller("QuestionCtrl",["$scope","$http","$routeParams","Shared","$location",function(a,c,e,g,d){g.requireLogin();a.Q={answerOptions:[]};C.log("Loaded QuestionCtrl...");e=e.qid;void 0!==e&&c.get("/assess/getQuestion.a?qid="+e).success(function(b){"OK"===b.Status?a.Q=b.Message:a.setStatus("Failed to load question.")}).error(function(b,f,c,d){a.setStatus(b.Message)});a.addOpt=function(){a.Q.answerOptions.push({})};a.save=function(){c.post("/assess/saveQuestion.a",a.Q).success(function(b){"OK"===
b.Status?(a.Q=b.Message,a.setStatus("Saved question!"),d.path("/Question/"+b.Message.questionId)):a.setStatus("Failed to save question!")}).error(function(b,f,c,d){a.setStatus(b.Message)})}}]);
srsApp.controller("AssessmentViewCtrl",["$scope","$http","$routeParams","Shared","$location",function(a,c,e,g,d){g.requireLogin();a.A={questions:[],responses:[]};g=0===d.path().indexOf("/EditSubmission")?"getAssessmentSubmission.a":"getAssessmentForTaking.a";c.get("/assess/"+g+"?id="+e.aid).success(function(b){"OK"===b.Status?a.A=b.Message:a.setStatus("Failed to process request.")}).error(function(b,f,c,d){a.setStatus(b.Message)});a.isSubmitted=function(){return void 0!==a.A.draft&&!a.A.draft};a.getMarkedAns=
function(){var b=[];a.A.questions.forEach(function(a,c,d){c=a.answerOptions.filter(function(b,c,d){return b.marked||"FTXT"===a.type&&void 0!==b.response});C.log("Marked: "+JSON.stringify(c));c.forEach(function(c){var d={};d.answer=c.answer;d.questionId=a.questionId;"FTXT"===a.type&&(d.answer=c.response);b.push(d)})});return{assessId:a.A.assessId,responses:b,submissionId:a.A.submissionId}};a.save=function(b){if(confirm("Sure you want to "+(b?"submit":"save")+"?")){var f=a.getMarkedAns();f.draft=!b;
C.log("Sending for save: "+JSON.stringify(f));c.post("/assess/saveAssessmentResponse.a",f).success(function(f){"OK"===f.Status?(a.A.submissionId=f.Message.submissionId,b?(a.setStatus("Submitted responses!"),d.path("/AssessmentResult/"+f.Message.submissionId)):(a.setStatus("Saved responses as a draft!"),d.path("/EditSubmission/"+f.Message.submissionId))):a.setStatus("Could not save data.")}).error(function(b,f,c,d){a.setStatus(b.Message)})}}}]);
srsApp.controller("AssessmentResultCtrl",["$scope","$http","$routeParams","Shared",function(a,c,e,g){g.requireLogin();a.A={questions:[],responses:[]};C.log("Getting assessment result...");c.get("/assess/getAssessmentResult.a?id="+e.sid).success(function(c){"OK"===c.Status?a.A=c.Message:a.setStatus(c.Message)}).error(function(c,b,f,e){a.setStatus(c.Message)});a.showAns=function(a,b){C.log("q="+JSON.stringify(a)+"\nopt="+JSON.stringify(b));return"FTXT"===a.type?b.answer===b.response:b.correct}}]);
...
b).success(function(b){"OK"===b.Status?a.Model.Comments=b.Message:a.setStatus("Failed to get comments.")})};a.loadTrailItem=function(c){if(void 0===c)a.RootModel.StatusMessage="Item not found!";else{C.log("Showing trailitem: "+JSON.stringify(c));$("#videoLoadMsg").show();a.Model.videoLoaded=!1;a.Model.TrailItem=c;C.log("w x h = "+b.innerWidth+" x "+b.innerHeight);var d="<iframe width='810' height='455.625' src='"+a.RootModel.getVideoUrl(c.url)+"'frameborder='0' allowfullscreen onload='$(\"#videoLoadMsg\").hide(\"slow\")'></iframe>";
