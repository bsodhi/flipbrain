...
//            $location.path("/Index");
    ...
    Shared.init = function () {
        C.log("Shared.init().");
        $http.get('/user/getSessionDetails.a')
                .success(function (data, status, headers, config) {
                    if (data.Status === "OK") {
                        ...
                        if (m.Subscriptions !== undefined)
                            Shared.Subscriptions = m.Subscriptions;
                        else
                            Shared.Subscriptions = [];

                    } else {
                        C.log("Shared.init(): Failed to load session data: " + JSON.stringify(data.Message));
                    }
                    $route.reload();
                    C.log("Shared.init(): Loaded User: " + JSON.stringify(Shared.User));
                })
